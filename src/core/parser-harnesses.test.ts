...
function setEnv(key: 'HOME' | 'USERPROFILE' | 'WSL_DISTRO_NAME' | 'WSL_INTEROP', value: string | undefined): void {
...
function withHome(setup: (home: string) => void, body: () => void): void {
  ...
  const prevWslDistroName = process.env.WSL_DISTRO_NAME;
  const prevWslInterop = process.env.WSL_INTEROP;
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-home-'));
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  // Keep these tests hermetic when the suite itself runs inside WSL: an empty
  // marker explicitly disables the kernel-release fallback in path discovery.
  ...
  try {
    ...
    setEnv('WSL_DISTRO_NAME', prevWslDistroName);
    setEnv('WSL_INTEROP', prevWslInterop);
